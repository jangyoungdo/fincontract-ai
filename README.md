...
![FinContract AI 기획 시스템 플로우](assets/fincontract-ai-architecture-v3.png)

## 구현 시스템 플로우

![FinContract AI 구현 시스템 플로우](assets/fincontract-ai-architecture-v6.svg)
...
- 단일 키워드가 아닌 위험 표현 조합과 19개 유형별 수정 방향·검토용 예시 문안 구현
...
결과 스키마 3.1에서 각 결정론 신호는 최초로 일치한 단어만 보여주지 않고 판단에 사용된 표현 조합을 함께 표시합니다. 수정 안내는 규칙 탐지와 분리된 `revision-guidance-v0.1.0` 결정론 템플릿으로 제공하며, `수정 방향`과 `검토용 예시 문안`을 구분합니다. 예시 문안은 계약별 기간·금액·상품 조건을 추정하지 않으며 실제 조건과 적용 법령을 확인한 뒤 확정해야 합니다.
