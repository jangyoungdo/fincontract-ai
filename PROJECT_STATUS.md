...
`전체 스택 MVP 구현 완료 — 공개 법령 RAG와 내부 의미 후보 검증 구현, 운영 평가 대기`
...
- `backend/app/rules/`: 합성 데이터로 검증하는 19개 위험 신호 규칙 기준선과 R04 복합 정상 예외 게이트
- `backend/app/services/`: 암호화 저장, 문서 처리, 법률 근거 RAG, 내부 decision-card RAG, 비동기 작업, 감사·보존, PDF 리포트
...
- 19개 규칙과 합성 decision card의 법률 전문가 검토 여부
- 실제 OpenAI API의 배포 환경별 보존 정책
...
4. 승인된 OpenAI 키로 Structured Outputs 연결 검증
